...
export default async function handler(req, res) {
  ...
  try {
    if (!cachedModule) {
      cachedModule = await fetchRemoteModule()
    }
    res.setHeader('Content-Type', 'application/javascript; charset=utf-8')
    res.setHeader('Cache-Control', 'public, max-age=3600, s-maxage=3600, stale-while-revalidate=86400')
    res.status(200).send(cachedModule)
  } catch (error) {
    console.error('[pdfjs/module] proxy error', error)
    res.setHeader('Content-Type', 'application/javascript; charset=utf-8')
    res.status(200).send(`throw new Error(${JSON.stringify(
      `Falha ao carregar PDF.js via proxy: ${error instanceof Error ? error.message : String(error)}`,
    )})`)
  }
}
