...
import { isValidBrazilPhone, isValidEmail, normalizePhone } from '../../validation/clientReadiness'
import { normalizeAndValidateDocument } from '../../normalize/document'
...
const RE_CITY_STATE_CEP_INLINE =
  /([A-ZÀ-ÝA-Za-zÀ-ÿ0-9\s'’.,•-]+)\s*\/\s*([A-Z]{2})[\s,\-–]*CEP[:\s]*([0-9]{5}-?[0-9]{3})/i
...
const RE_EMAIL_CANDIDATE = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi
const RE_PHONE_CANDIDATE = /(?:\+?55\s*)?(?:\(?\d{2}\)?\s*)?(?:9?\d{4})[-.\s]?\d{4}/g
...
function sanitizeCity(value: string | null | undefined): string | null {
  ...
  if (!/[aeiouáàâãéêíóôõú]/i.test(normalized)) return null
  if (/\b[A-Za-zÀ-ÿ]\b/.test(normalized)) return null
  return normalized
}
...
function extractValidatedEmail(text: string): string | null {
  const matches = text.match(RE_EMAIL_CANDIDATE) ?? []
...
function extractValidatedPhone(text: string): string | null {
  const matches = text.match(RE_PHONE_CANDIDATE) ?? []
  for (const candidate of matches) {
    if (isValidBrazilPhone(candidate)) {
...
function extractValidatedDocument(candidate: string | null | undefined): string | null {
  if (!candidate) return null
  const normalized = normalizeAndValidateDocument(candidate)
  return formatCpfCnpj(normalized)
}
...
export function extractContractFields(text: string): ParsedContractFields {
  ...
  return {
    ...
    contractorDocument: extractValidatedDocument(contractorBlock.match(RE_CONTRACTOR_DOCUMENT)?.[1] ?? text.match(RE_CONTRACTOR_DOCUMENT)?.[1] ?? null),
    contractorEmail: extractValidatedEmail(contactBlock) ?? extractValidatedEmail(text),
    contractorPhone: extractValidatedPhone(contactBlock) ?? extractValidatedPhone(text),
