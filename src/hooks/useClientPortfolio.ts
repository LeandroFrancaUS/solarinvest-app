...
export function useClientPortfolio(): UseClientPortfolioResult {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('portfolio:changed', onPortfolioChanged as EventListener)
    }

    return () => {
      if (typeof window !== 'undefined') {
        window.removeEventListener('portfolio:changed', onPortfolioChanged as EventListener)
      }
    }
  }, [load])
