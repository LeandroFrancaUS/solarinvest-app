...
export function PdfImportDialog({
  ...
  const processFile = useCallback(
    async (file: File, bypassUnsavedWarning = false) => {
      ...
      // ---- PDF branch — PDFs do not contain embedded import data ----
      // Proposals generated by SolarInvest store import data in a companion .json file.
      // Attempting to extract text from the PDF would only result in the same error after a
      // slow, expensive processing step. Short-circuit immediately with a helpful message.
      setState({
        phase: 'error',
        message:
          'Arquivos PDF não podem ser usados para importação. ' +
          'Utilize o arquivo .json gerado junto com a proposta. ' +
          'Para baixar os dados de uma proposta já gerada, abra-a na pré-visualização e clique em "Baixar dados (.json)".',
      })
    },
    [advanceToComparison, hasUnsavedChanges],
  )
        ...
        {phase === 'file-pick' && (
          <div className="modal-body">
            <p className="pdf-import-modal__description">
              ...
              importar as informações de volta ao formulário.
            </p>
            <div
            ...
            >
              ...
              <span className="pdf-import-dropzone__hint">.json · máx. 40 MB</span>
              <input
                ref={fileInputRef}
                type="file"
