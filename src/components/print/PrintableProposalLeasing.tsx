...
function PrintableProposalLeasingInner(
  ...
  const resumoProposta = [
    ...
    },
    {
      ...
      value: inicioOperacaoTexto ? `${inicioOperacaoTexto} · Até 60 dias após assinatura` : 'Até 60 dias após assinatura',
    },
    {
          ...
          <section className="print-section keep-together avoid-break">
            ...
            <tbody>
              {mensalidadesPorAno.map((linha, index) => {
                ...
                const isMensalidadeZeroPosContrato = isPosPrazo && linha.mensalidade === 0
                  ...
                  <tr key={`mensalidade-${linha.ano}`} className={rowClassName || undefined}>
                    ...
                    <td
                      className={[
                        ...
                        isMensalidadeZeroPosContrato ? 'leasing-zero-highlight' : undefined,
                      ]
                        .filter(Boolean)
                        .join(' ')}
                    >
                      <span
                        className={isMensalidadeZeroPosContrato ? 'leasing-zero-pill' : undefined}
                        aria-label={isMensalidadeZeroPosContrato ? 'Mensalidade zerada após o contrato' : undefined}
                      >
