...
function resolveStartDate(row: PortfolioClientRow): Date | null {
  const raw = row.first_billing_date ?? row.inicio_da_mensalidade ?? row.commissioning_date_billing ?? row.billing_start_date ?? row.contract_start_date
  if (!raw) return null
...
function dueDateForInstallment(row: PortfolioClientRow, number: number): Date | null {
  ...
  const dueDay = resolveDueDay(row)
...
function buildCharges(row: PortfolioClientRow): Array<MonthlyChargeLike & { value: number }> {
  ...
  if (installments.length > 0) {
    return installments.map((inst, index) => {
      ...
      const due = dueDateForInstallment(row, number)
      return {
        number,
        dueDate: due,
        status: inst.status,
...
export function computeDashboardFinanceKPIs(
  ...
  for (const row of portfolio) {
    ...
    const charges = buildCharges(row)
