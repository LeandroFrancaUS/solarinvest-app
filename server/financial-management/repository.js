...
/**
 * Builds SQL date-range conditions.
 * Returns an object with `fromClause` and `toClause` fragments plus bindings.
 */
...
export async function listFinancialEntries(sql, { from, to } = {}) {
  const { from: fromDate, to: toDate } = buildDateRange(from, to)
  ...
  const rows = await sql`
    ...
    WHERE deleted_at IS NULL
      ${fromDate ? sql`AND competence_date >= ${fromDate}` : sql``}
      ${toDate ? sql`AND competence_date <= ${toDate}` : sql``}
 ...
 * Combines data from project_financial_snapshots (project metrics)
 * and financial_entries (company entries).
 */
export async function getFinancialSummary(sql, { from, to } = {}) {
  const { from: fromDate, to: toDate } = buildDateRange(from, to)

  // Aggregate from project_financial_snapshots
...
export async function listFinancialProjects(sql, { from, to } = {}) {
  ...
  const rows = await sql`
    ...
    WHERE TRUE
      ${fromDate ? sql`AND pfs.created_at >= ${fromDate}` : sql``}
      ${toDate ? sql`AND pfs.created_at <= ${toDate}` : sql``}
