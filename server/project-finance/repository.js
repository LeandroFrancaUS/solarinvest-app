...
export async function findProjectFinanceByProjectId(sql, projectId) {
  // First try the full query (including columns added in migration 0048).
  // If those columns do not yet exist in the DB (migration pending), catch the
  // PostgreSQL "undefined_column" error (code 42703) and retry with a fallback
  // query that omits them — avoids a 500 when the migration hasn't been applied.
  try {
    ...
    // 42703 = PostgreSQL undefined_column — migration 0048 not yet applied.
    if (err?.code === '42703' || (err?.message ?? '').includes('override_payload_json') || (err?.message ?? '').includes('technical_params_json')) {
      console.warn('[project-finance] columns from migration 0048 missing — retrying without them', { code: err?.code })
      const rows = await sql`
        SELECT
          pfp.id::text,
          pfp.project_id::text,
...
/**
 * Resolves the contract type, contractual term and base monthly fee for a
 * project. Returns `{ contract_type, contract_term_months, mensalidade_base }`
 * or `null` when the project cannot be found.
 *
 * `mensalidade_base` is sourced from `client_energy_profile.mensalidade`
...
export async function upsertProjectFinance(sql, projectId, fields, userId = null) {
  ...
  // Attempt the full upsert (with columns from migration 0048).
  // On 42703 (undefined_column), retry without override_payload_json / technical_params_json.
  try {
    const rows = await sql`
    ...
    // 42703 = PostgreSQL undefined_column — migration 0048 not yet applied.
    if (err?.code === '42703' || (err?.message ?? '').includes('override_payload_json') || (err?.message ?? '').includes('technical_params_json')) {
      console.warn('[project-finance] columns from migration 0048 missing — retrying upsert without them', { code: err?.code })
      const rows = await sql`
        INSERT INTO project_financial_profiles (
          project_id,
          client_id,
