...
export async function ensureOperationalSchema(sql) {
  ...
  ensurePromise = (async () => {
    try {
      ...
      // In some managed Neon environments extension DDL may be restricted.
      // Keep runtime alive and rely on existing schema/migrations.
      console.warn('[schema] pgcrypto extension ensure skipped:', error?.message ?? error)
    }
