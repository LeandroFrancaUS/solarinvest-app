...
export async function createClient(sql, data) {
  ...
  const runCreate = async (modernSchema) => {
    if (modernSchema) {
      return sql`
        INSERT INTO clients (
          client_name, client_document, cpf_normalized, cpf_raw,
          cnpj_normalized, cnpj_raw, document_type,
          client_phone, client_email, client_city, client_state, client_address, cep, uc_geradora, uc_beneficiaria, system_kwp, term_months, consumption_kwh_month, distribuidora,
          ...
          ${cnpj_normalized}, ${cnpj_raw}, ${document_type},
          ${phone}, ${email}, ${city}, ${state}, ${address}, ${cep}, ${uc}, ${uc_beneficiaria}, ${system_kwp}, ${term_months}, ${consumption_kwh_month}, ${distribuidora},
  ...
  try {
    const rows = await runCreate(true)
    return rows[0] ?? null
  } catch (error) {
    const code = error?.code ?? null
    const message = error instanceof Error ? error.message : String(error)
    const missingColumn = code === '42703' || message.includes('client_') || message.includes('consumption_kwh_month')
    if (!missingColumn) throw error
    console.warn('[clients][create] retrying with legacy schema compatibility mode')
    const rows = await runCreate(false)
    return rows[0] ?? null
  }
}
...
export async function updateClient(sql, clientId, data, options = {}) {
  ...
  const ownerClause = scopeByOwner ? 'AND owner_user_id = $22' : ''
  const params = [
    name ?? null,
    phone ?? null,
    email ?? null,
  ...
  const runUpdate = async (modernSchema) => {
    if (modernSchema) {
      return sql(
        `UPDATE clients SET
           ...
           document_type    = COALESCE($18, document_type),
           identity_status  = COALESCE($19, identity_status),
           metadata         = CASE
                                WHEN $20::jsonb IS NOT NULL
                                THEN COALESCE(metadata, '{}'::jsonb) || $20::jsonb
                                ELSE metadata
                              END,
           updated_at       = now()
         WHERE id = $21
    ...
    const ownerClauseLegacy = scopeByOwner ? 'AND owner_user_id = $18' : ''
    const legacyParams = [
      name ?? null,
      phone ?? null,
      email ?? null,
  ...
  try {
    ...
    const missingColumn = code === '42703' || message.includes('client_') || message.includes('consumption_kwh_month')
    if (!missingColumn) throw error
    console.warn('[clients][update] retrying with legacy schema compatibility mode')
...
export async function listClients(sql, filter = {}) {
  ...
  if (search) {
    ...
    conditions.push(`(c.client_name ILIKE $${idx} OR c.cpf_normalized ILIKE $${idx} OR c.cnpj_normalized ILIKE $${idx} OR c.client_email ILIKE $${idx} OR c.client_phone ILIKE $${idx} OR c.uc_geradora ILIKE $${idx} OR c.uc_beneficiaria ILIKE $${idx} OR c.client_city ILIKE $${idx} OR c.client_state ILIKE $${idx} OR c.client_address ILIKE $${idx} OR c.cep ILIKE $${idx} OR c.distribuidora ILIKE $${idx})`)
  }
