...
// ensures the DDL runs at most once per server process.  All statements
// use CREATE TABLE / INDEX IF NOT EXISTS and are therefore completely
// idempotent — running them on an already-migrated database is a no-op.
//
// Policy notes
// ------------
// The INSERT/UPDATE policies here use can_access_owner() (not can_write_owner())
// for the 8 lifecycle tables, incorporating the fix from migration 0028.
// client_energy_profile keeps the original can_write_owner() policies from
// migration 0025.
...
async function _applySchema() {
  ...
  `)
  await exec(sql, `ALTER TABLE public.client_energy_profile ENABLE ROW LEVEL SECURITY`)
  await exec(sql, `
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public' AND tablename = 'client_energy_profile'
  ...
  `)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_lifecycle_client_id ON public.client_lifecycle (client_id)`)
  await exec(sql, `
  ...
  `)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_contracts_client_id ON public.client_contracts (client_id)`)
  await exec(sql, `
  ...
  `)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_project_status_client_id ON public.client_project_status (client_id)`)
  ...
  `)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_billing_profile_client_id ON public.client_billing_profile (client_id)`)
  ...
  `)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_billing_installments_client_id ON public.client_billing_installments (client_id)`)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_billing_installments_due_date ON public.client_billing_installments (due_date)`)
  await exec(sql, `
  ...
  `)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_notes_client_id ON public.client_notes (client_id)`)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_notes_created_at ON public.client_notes (client_id, created_at DESC)`)
  ...
  `)
  await exec(sql, `CREATE INDEX IF NOT EXISTS idx_client_reminders_client_id ON public.client_reminders (client_id)`)
  await exec(sql, `
  ...
  // ── Enable RLS on all 8 client-management tables ──────────────────────────
  const mgmtTables = [
    'client_lifecycle',
    'client_contracts',
  ...
  for (const tbl of mgmtTables) {
    await exec(sql, `ALTER TABLE public.${tbl} ENABLE ROW LEVEL SECURITY`)
  }

  // ── RLS policies for client-management tables (0027 + 0028 combined) ──────
  // INSERT/UPDATE use can_access_owner (the fix from migration 0028).
  // SELECT/DELETE use can_access_owner / can_write_owner respectively.
  await exec(sql, `
    DO $$
    DECLARE
      tbl TEXT;
    ...
    BEGIN
      FOREACH tbl IN ARRAY tables LOOP
        ...
        -- INSERT (can_access_owner — 0028 fix applied from the start)
        IF NOT EXISTS (
          SELECT 1 FROM pg_policies
        ...
        -- UPDATE (can_access_owner — 0028 fix applied from the start)
        IF NOT EXISTS (
          SELECT 1 FROM pg_policies
  ...
  // Also apply the 0028 fix to any existing policies that used can_write_owner
  // for INSERT/UPDATE (in case the tables existed but 0028 had not been applied).
  await exec(sql, `
    DO $$
    DECLARE
      tbl TEXT;
    ...
    BEGIN
      FOREACH tbl IN ARRAY tables LOOP
        -- Replace can_write_owner INSERT with can_access_owner version
