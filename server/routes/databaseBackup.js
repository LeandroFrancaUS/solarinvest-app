...
function normalizeImportSelection(selection, fallbackList = []) {
  ...
  return selection
    .filter((entry) => entry && typeof entry === 'object' && entry.selected !== false)
...
export async function handleDatabaseBackupImportRequest(req, res, { sendJson, body }) {
  ...
  try {
    ...
    sendJson(res, 200, {
      ok: true,
      report: {
        clientsInserted: importResult.importedClients,
        clientsUpdated: 0,
        clientsSkipped: 0,
        proposalsInserted: importResult.importedProposals,
        proposalsUpdated: 0,
        proposalsSkipped: 0,
        failures: [],
      },
